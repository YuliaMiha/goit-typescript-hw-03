...
// зберігає їх у приватному властивості key. Клас Person повинен мати метод getKey, який 
// повертає збережений ключ.
...
// У цьому класі також повинен бути метод comeIn, який додає об'єкт класу Person у масив 
// tenants, якщо door відкрита. Ваш абстрактний клас House також повинен мати абстрактний метод
...
// Реалізуйте метод openDoor у цьому класі. Якщо ключ, переданий цьому методу, збігається з
//  ключем, збереженим як key, то двері відчиняються.
...
abstract class House {
    ...
    comeIn(person: Person): Person[]{
        if(this.door){
            ...
            console.log("Увійшов(ла) в будинок.");
        }else{
...
class MyHouse extends House {
    ...
    openDoor(key:Key):boolean{
      const  fromMyKey = this.key.getSignature(); // з class MyHouse, тобто House
      const fromKey = key.getSignature(); // з class Key
        if(fromMyKey === fromKey){
            this.door = true
        }else{
             this.door = false
        }
